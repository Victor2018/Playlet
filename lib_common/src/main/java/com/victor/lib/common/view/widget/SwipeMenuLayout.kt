...
/*                    if (!mScroller.isFinished()) {
                ...
                MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                    ...
                    if (!iosInterceptFlag) { //且滑动了 才判断是否要收起、展开menu
                        ...
                        val velocityX = mVelocityTracker?.getXVelocity(mPointerId) ?: 0f
    ...
    private fun cancelAnim() {
        var isCloseAnimRunning = mCloseAnim?.isRunning ?: false
        if (isCloseAnimRunning) {
            mCloseAnim?.cancel()
        }
        var isExpandAnimRunning = mExpandAnim?.isRunning ?: false
        if (isExpandAnimRunning) {
            mExpandAnim?.cancel()
        }
    }
