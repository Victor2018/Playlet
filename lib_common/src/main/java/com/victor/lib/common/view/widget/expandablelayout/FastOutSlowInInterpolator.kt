 ...
 * -----------------------------------------------------------------
 * File: FastOutSlowInInterpolator
 * Author: Victor
...
class FastOutSlowInInterpolator : Interpolator {
     ...
     * 201 points.
     */
    private val mValues = floatArrayOf(
    ...
    override fun getInterpolation(input: Float): Float {
        ...
        // Calculate values to account for small offsets as the lookup table has discrete values
        val quantized: Float = position * mStepSize
        val diff: Float = input - quantized
        val weight: Float = diff / mStepSize

        // Linearly interpolate between the table values

        // Linearly interpolate between the table values
        return mValues.get(position) + weight * (mValues.get(position + 1) - mValues.get(position))
    }
}